...
app.post('/api/debug-enrich-specific', async (req, res) => {
  try {
    // 1. Search GID for "Full set Air Vent Covers for Toyota Landcruiser 79 Series"
    const searchRes = await client.request(`
      query {
        products(first: 5, query: "title:'Full set Air Vent Covers for Toyota Landcruiser 79 Series'") {
          edges {
            node {
              id
              title
    ...
    const matches = searchRes.data.products.edges;
    let foundId = null;
    if (matches.length > 0) {
      foundId = matches[0].node.id;
      console.log(`🔍 Found GID for Air Vent Covers: ${foundId}`);
    } else {
      console.warn("⚠️ Could not find GID for 'Full set Air Vent Covers for Toyota Landcruiser 79 Series'");
    }

    const targetIds = [
      "gid://shopify/Product/10300979020067",
      "gid://shopify/Product/10300976365859",
      "gid://shopify/Product/10300975055139",
      "gid://shopify/Product/10300974956835"
    ];
    ...
    for (const pid of targetIds) {
      try {
        // Force re-enrichment by calling enrichProduct.
        // Wait, enrichProduct checks if product description contains Specifications table.
        // Since we want to FORCE enrichment for these specific products (to update their metafields with the new prompt taxonomy),
        // we temporarily bypassed the Specifications check by passing an extra parameter, or we can just run it.
        // Wait! Let's check: the specifications table is already present on some?
        // Wait, on the 4 window strips, they do NOT have Specifications table yet! They are new!
        // For the Air Vent Covers: does it have it? Let's check when it runs.
        // In order to make sure we force it, let's write a small wrapper or just run enrichProduct.
        const success = await enrichProduct(client, openai, pid, false);
        results.push({ id: pid, success });
      } catch (err) {
        console.error(`Error enriching product ${pid}:`, err.message);
