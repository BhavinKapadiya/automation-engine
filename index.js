...
app.post('/api/test-rich-descriptions', async (req, res) => {
  try {
    ...
    console.log("🚀 Fetching collections and product listings from Shopify...");
    const getRes = await client.request(GET_COLLECTIONS);
    const collections = getRes.data.collections.edges.map(e => e.node);

    const results = [];

    for (const col of collections) {
      console.log(`✍️ Analyzing ${col.products.edges.length} products for Collection: "${col.title}"`);
      const productList = col.products.edges.map((e, index) => `${index + 1}. ${e.node.title} (${e.node.productType || 'Part'})`).join("\n");
...
5. Output format: Return the paragraph as a clean string. Do not wrap in JSON.`;
      ...
      try {
        ...
        results.push({
          ...
          count: col.products.edges.length
        });
      } catch (err) {
        results.push({
          title: col.title,
